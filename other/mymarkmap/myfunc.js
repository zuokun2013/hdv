...
// const reqUrl = 'https://raw.githubusercontent.com/zuokun2013/lsn/main/pages/%E5%89%8D%E8%A1%8C%E7%B3%BB%E5%88%973%E5%9B%A0%E6%9E%9C%E4%B8%8D%E8%99%9A.md'
const reqUrl = './data.md'
var markdown;			
...
const resPromise = fetch(reqUrl);

      resPromise.then((response) => response.text())
      .then(PromiseResult => {
        
        markdown = PromiseResult;
        console.log(markdown);
        console.log(`Received response: ${markdown}`);
...
function renderMMNodeWithPath() {
  ...
  if (currNode != null) {
    ...
    const currPath = currNode.state.path;
    
    

    const parentPath = currPath.substr(0, currPath.lastIndexOf('.')+1);
    console.log(`parentPath=${parentPath}`)
    // mm.setData(currNode);
    const currDepth = currNode.depth;

    mmNodeMap.forEach(elem=> {
      if(elem.state.path.startsWith(parentPath) && elem.depth == currDepth-1) {
        elem.payload.fold = false;
        console.log(`elem=${elem.state.path}`);
      } else {
        elem.payload.fold = true;
      }
    });

    currPath.split(".").forEach(element => {
      // console.log(element);
      // console.log(mmNodeMap.get(element));
      console.log(mmNodeMap.get(element).state.path);
